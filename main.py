...
def letter_seg(lines_img, x_lines, i):
	...
	for e in range(len(letter)):
		if(letter[e][0]<x_linescopy[0]):
			...
			# letter_img = cv2.resize(letter_img_tmp, dsize =(2880, 1640), interpolation = cv2.INTER_AREA)
			letter_img = cv2.resize(letter_img_tmp, dsize =(5000, 5000), interpolation = cv2.INTER_AREA)
			cv2.imwrite('./segmented_chars/img1/'+str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.png', 255-letter_img)
			# applying filters
			
			ImageOps.expand(Image.open('./segmented_chars/img1/'+str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.png'),border=2000,fill='white').save('./segmented_chars_border/img1/'+str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.png')

			# adding white borders to image
			char_image = Image.open('./segmented_chars/img1/'+str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.png')
			white = Image.open('./white.png')
			
			white.paste(char_image)
			white.save('./border_img/img1/'+str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.png')
...
# -------old code for getting files manually from directory------#
...
def characterSegmentation():
	...
	input_image = "segmented_words_border/"+input_file
	
	print(input_image)
	
	print("\n........Program Initiated.......\n")
	global src_img
	src_img= cv2.imread(input_image, 1)
	copy = src_img.copy()
